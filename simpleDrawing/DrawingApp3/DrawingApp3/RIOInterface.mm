...
static OSStatus	AudioUnitRenderCallback (void *inRefCon,
    ...
    RIOInterface* audioUnitWrapper = (RIOInterface *)inRefCon;
    
    OSStatus err = AudioUnitRender(audioUnitWrapper->ioUnit, ioActionFlags, inTimeStamp, 1, inNumberFrames, ioData);
...
#define LOWPASSFILTERTIMESLICE .001
    ...
    //
    //
    //    SInt16* samples = (SInt16*)(THIS->bufferList->mBuffers[0].mData); // Step 1: get an array of your samples that you can loop through. Each sample contains the amplitude.
    //
    //    Float32 decibels = DBOFFSET; // When we have no signal we'll leave this on the lowest setting
    //    Float32 currentFilteredValueOfSampleAmplitude, previousFilteredValueOfSampleAmplitude; // We'll need these in the low-pass filter
    //    Float32 peakValue = DBOFFSET; // We'll end up storing the peak value here
    ...
    //
    //    NSLog(@"decibel level is %f", decibels);
    //    //[THIS->listener ampChangedValue:(decibels)];
    //
    //    for (UInt32 i=0; i < THIS->bufferList->mNumberBuffers; i++) { // This is only if you need to silence the output of the audio unit
    //        memset(THIS->bufferList->mBuffers[i].mData, 0, THIS->bufferList->mBuffers[i].mDataByteSize); // Delete if you need audio output as well as input
    //    }
    //    }else{
